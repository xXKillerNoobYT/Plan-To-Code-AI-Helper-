````markdown
# You need to write unit tests with Vitest for the function or function component `<<<FunctionName>>>` in a new spec file (<<<SpecFileName>>>).
# Guidelines:
...
- Mock all the dependent functions and classes from other module or files in the test cases using `vi.mock()`.
- Use `vi.spyOn()` instead of `jest.spyOn()`.
- Use `vi.fn()` instead of `jest.fn()`.
- Mock the timezones when necessary using `vi.setSystemTime()`!!!
...
- Do NOT call vi.spyOn(React, ...) under any circumstances.
